...
inline void MinimaxBot::_make(int q, int r) {
    ...
    if (_need_mix) _mix_apply(ccq, ccr, ncc, -1.f);
...
inline void MinimaxBot::_init_eval_arrays() {
    ...
    // Mixnet planes: full rebuild from _lp. Zero-anchored codebook means
    // the memset-zero state is exact for every untouched cell.
    if (_need_mix) {
        std::memset(_amem, 0, sizeof(_amem));
        std::memset(_usup, 0, sizeof(_usup));
        std::memset(_acc3, 0, sizeof(_acc3));
        _ucnt = 0;
        _mix_pkey = ~0ull;
        if (!_board_cells.empty()) {
            int q0 = ARR, q1 = -1, r0 = ARR, r1 = -1;
            ...
            q0 = std::max(q0 - LP_CENTER - 1, 1);
            q1 = std::min(q1 + LP_CENTER + 1, ARR - 2);
            r0 = std::max(r0 - LP_CENTER - 1, 1);
            r1 = std::min(r1 + LP_CENTER + 1, ARR - 2);
            for (int qi = q0; qi <= q1; qi++)
                for (int ri = r0; ri <= r1; ri++) {
                    if (!_mix_active(qi, ri)) continue;
                    _mix_amem_refresh(qi, ri);
                    for (int k = 0; k < 7; k++)
                        _usup[qi + MXO_Q[k]][ri + MXO_R[k]]++;
                }
            for (int qi = q0 - 1; qi <= q1 + 1; qi++)
                for (int ri = r0 - 1; ri <= r1 + 1; ri++) {
                    if (_usup[qi][ri] == 0) continue;
                    _ucnt++;
                    _mix_fprime(qi, ri, +1.f);
                }
        }
    }
}
