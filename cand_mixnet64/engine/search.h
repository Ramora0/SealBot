...
inline MoveResult MinimaxBot::get_move(const GameState& gs) {
    ...
    if (_need_mix) {
        std::memcpy(_saved->amem, _amem, sizeof(_amem));
        std::memcpy(_saved->usup, _usup, sizeof(_usup));
        std::memcpy(_saved->acc3, _acc3, sizeof(_acc3));
        _saved->ucnt = _ucnt;
    }
    ...
    for (int depth = 1; depth <= max_depth; depth++) {
        try {
            ...
            if (_need_mix) {
                std::memcpy(_amem, _saved->amem, sizeof(_amem));
                std::memcpy(_usup, _saved->usup, sizeof(_usup));
                std::memcpy(_acc3, _saved->acc3, sizeof(_acc3));
                _ucnt = _saved->ucnt;
                _mix_pkey = ~0ull;
            }
